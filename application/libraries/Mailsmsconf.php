    ...
    public function mailsms($send_for, $sender_details, $date = NULL, $exam_schedule_array=NULL) {
		...
		if (!empty($chk_mail_sms)) {
            if ($send_for == "student_admission") {
                ...
                if ($chk_mail_sms['sms']) {
                    $this->CI->customsms->sentRegisterSMS($sender_details['student_id'], $sender_details['contact_no']);
				}
                ...
                if ($chk_mail_sms['sms']) {
					$this->CI->customsms->sendLoginCredential($chk_mail_sms, $sender_details);
				}
                ...
                if ($chk_mail_sms['sms']) {
                    $this->CI->customsms->sentAddFeeSMS($invoice->invoice_id, $invoice->sub_invoice_id, $sender_details['contact_no']);
                }
    ...
    public function sendResult($chk_mail_sms, $exam_result, $exam_schedule_array) {
		...
		if ($chk_mail_sms['mail'] OR $chk_mail_sms['sms']) {
            ...
            if ($chk_mail_sms['sms']) {
				...
				$this->CI->customsms->sentExamResultSMS($detail);
            }
            }
        }
    }
