...
class Studentfeemaster_model extends MY_Model {
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null) {
        ...
        $sql .= " AND `students`.`school_id`=".$this->school_id;
    ...
    public function fee_deposit($data, $send_to) {
        ...
        if ($q->num_rows() > 0) {
            ...
            $data['school_id'] = $this->school_id;
            $res_no = $this->db->select('res_no')->from('student_fees_deposite')->where('school_id',$this->school_id)->order_by('id','DESC')->limit(1)->get()->row_array();
            $data['res_no'] = $res_no['res_no']+1;
    ...
    function getTransFeeBetweenDate($start_date, $end_date)
    {
        $this->db->select('std_transport_fee.*,students.firstname,lastname,father_name');
        $this->db->from('std_transport_fee');  
        $this->db->join('students','std_transport_fee.student_id=students.id');     
        $this->db->where('std_transport_fee.school_id',$this->school_id);
        $query = $this->db->get();
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $key => $value) {
                $return = $this->findTransObjectByid($value, $st_date, $ed_date);
                if (!empty($return)) {
                    foreach ($return  as $k => $v) {
                        ...
                        $a['amount'] = $v['amount'];
                        $a['date'] = $v['date'];
                        if (isset($v['concession'])) {
                            $a['concession'] = $v['concession'];
                        }else{
                            $a['concession'] = 0;
                        }
                        $a['description'] = $v['description'];
                        $a['payment_mode'] = $v['payment_mode'];
                        $return_array[] = $a;
                    }
                }
            }
    ...
    function findTransObjectByid($array, $st_date, $ed_date) {

        $ar = unserialize($array['amount_data']);
        $array = array();
        for ($i = $st_date; $i <= $ed_date; $i += 86400) {
            $find = date('Y-m-d', $i);
            foreach ($ar as $row_key => $row_value) {
                //var_dump($row_value['date']);die;
                if (date('Y-m-d',strtotime($row_value['date'])) == $find) {
                    $array[] = $row_value;
                }
            }
        }
