...
class Studentsession_model extends MY_Model {
    ...
    public function getTotalStudentBySession() {
		$query = "SELECT count(*) as `total_student` FROM `student_session` INNER JOIN students on students.id=student_session.student_id LEFT JOIN transfer_certificate ON students.id=transfer_certificate.student_id  where transfer_certificate.student_id IS NULL AND  student_session.session_id=".$this->db->escape($this->current_session).' and student_session.school_id='.$this->school_id;
