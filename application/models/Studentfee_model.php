...
class Studentfee_model extends MY_Model {
    ...
    public function getTransportFee($id){
        ...
        $this->db->where('student_session_id',$this->current_session);
        $this->db->where('school_id',$this->school_id);
        $query = $this->db->get();
        return $query->row_array();
    }
    public function addTransportFee($data){
        if(isset($data['id'])){
            $this->db->where('id', $data['id']);
            $this->db->update('std_transport_fee', $data);
        }else{
            $data['school_id'] = $this->school_id;
            $data['student_session_id'] = $this->current_session;
            $this->db->insert('std_transport_fee', $data); 
    ...
    public function getStudentTotalFee($class_id, $student_session_id) {
        $query = "SELECT a.totalfee,b.fee_deposit,b.payment_mode  FROM ( SELECT COALESCE(sum(amount),0) as totalfee FROM `feemasters` WHERE school_id=".$this->db->escape($this->school_id)." session_id =$this->current_session and class_id=" . $this->db->escape($class_id) . ") as a, (select COALESCE(sum(amount),0) as fee_deposit,payment_mode from student_fees WHERE student_session_id =" . $this->db->escape($student_session_id) . ") as b";
        $query = $this->db->query($query);
        return $query->row();
    }
    public function getLastYearFee($data)
    {
        $fee_amount = 0;
        $deposite  = 0;
        $descount = 0;
        foreach ($data as $groupkey => $groupvalue) {
             $fees = $groupvalue->fees;
             foreach ($fees as $feeskey => $feesvalue) {
                $fee_amount =$fee_amount+$feesvalue->amount;
                $installment = $feesvalue->amount_detail;
                if ($installment!='' && !empty($installment)) {
                    $installment = json_decode($installment);
                    foreach ($installment as $depositekey => $depositevalue) {
                        $deposite = $deposite+$depositevalue->amount;
                        $descount = $descount+$depositevalue->amount_discount;
                    }
                }
        ...
        $total = $fee_amount - ($deposite + $descount);
        return $total;
